...
Treebot is a chatbot interface that lets users fork or handoff from any point in a conversation, creating a branching tree of chats. Chats are organized into workspaces so you can group related threads. The goal is to make learning conversations easier by letting you explore alternative paths without losing context.
...
- **Data layer:** SQLite + Drizzle ORM with tables for users, API keys, workspaces, chats (including parent-child links), and invite codes.
